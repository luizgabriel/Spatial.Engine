...
public:
	System(Application& application) : m_app{&application}, m_system{} { connect(application, &m_system); }

	template <typename... Args>
	System(Application& application, Args&&... args) : m_app{&application},
													   m_system{std::forward<Args>(args)...}
	{
		connect(*m_app, &m_system);
	}

	~System() { disconnect(*m_app, &m_system); }

	System(const System& other) = delete;

	System(System&& other) noexcept : m_app(other.m_app), m_system(std::move(other.m_system)) {}
