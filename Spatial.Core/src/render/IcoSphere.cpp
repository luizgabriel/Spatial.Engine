...
const IcoSphere::VertexList IcoSphere::s_vertices = {
    {-X, N, Z}, {X, N, Z}, {-X, N, -Z}, {X, N, -Z}, {N, Z, X}, {N, Z, -X}, {N, -Z, X}, {N, -Z, -X}, {Z, X, N}, {-Z, X, N}, {Z, -X, N}, {-Z, -X, N}};

const IcoSphere::TriangleList IcoSphere::s_triangles = {
...
{
    m_mesh = makeIcosphere(subdivisions);
}

IcoSphere::Index IcoSphere::vertexForEdge(
...
IcoSphere::TriangleList IcoSphere::subdivide(VertexList &vertices, TriangleList const &triangles)
{
    ...
    for (Triangle const &each : triangles)
    {
        ...
        mid[0] = vertexForEdge(lookup, vertices, each.vertex[0], each.vertex[1]);
        mid[1] = vertexForEdge(lookup, vertices, each.vertex[1], each.vertex[2]);
        mid[2] = vertexForEdge(lookup, vertices, each.vertex[2], each.vertex[0]);
...
IcoSphere::IndexedMesh IcoSphere::makeIcosphere(int subdivisions)
{
    VertexList vertices = s_vertices;
    TriangleList triangles = s_triangles;
