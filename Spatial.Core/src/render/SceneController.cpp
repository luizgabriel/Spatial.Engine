...
{
	registry.getEntities<const ecs::SceneView>(ecs::ExcludeComponents<Scene>).each([&](ecs::Entity entity, const auto&) {
		registry.addComponent<Scene>(entity, createScene(mEngine));
	});
