...
namespace spatial::ui {
    ...
    void ImGuiRenderer::setViewport(const math::uvec2 &windowSize, const math::uvec2 &frameBufferSize) {
        const auto dpiScaleX =
                windowSize.x > 0 ? static_cast<float>(frameBufferSize.x) / static_cast<float>(windowSize.x) : 1.0f;
        const auto dpiScaleY =
                windowSize.y > 0 ? static_cast<float>(frameBufferSize.y) / static_cast<float>(windowSize.y) : 1.0f;
        ...
        ImGui::SetCurrentContext(mImguiContext);
        ui::imguiRefreshViewport(static_cast<float>(windowSize.x), static_cast<float>(windowSize.y), dpiScaleX,
                                 dpiScaleY);
    }
 ...
 */
    void ImGuiRenderer::renderDrawData() {
        ...
        builder.boundingBox({{0,     0,     0},
                             {10000, 10000, 10000}}).culling(false);
