								   ...
								   const ecs::Registry& registry) const
{
	...
	instance->setParameter("albedo", albedoTexture.value_or(*dummyWhite), gDefaultSampler);
	instance->setParameter("tilingOffset", math::vec4{tiling, offset});

	instance->setParameter("metallic", metallic);
	auto metallicTexture = graphics::getTexture(registry, metallicMap);
	instance->setParameter("metallicMap", metallicTexture.value_or(*dummyWhite), gDefaultSampler);

	instance->setParameter("roughness", roughness);
	auto roughnessTexture = graphics::getTexture(registry, roughnessMap);
	instance->setParameter("roughnessMap", roughnessTexture.value_or(*dummyWhite), gDefaultSampler);

	instance->setParameter("reflectance", reflectance);
	auto reflectanceTexture = graphics::getTexture(registry, reflectanceMap);
	instance->setParameter("reflectanceMap", reflectanceTexture.value_or(*dummyWhite), gDefaultSampler);

	auto ambientOcclusionMapTexture = graphics::getTexture(registry, ambientOcclusionMap);
	instance->setParameter("ambientOcclusionMap", ambientOcclusionMapTexture.value_or(*dummyWhite), gDefaultSampler);

	auto normalMapTexture = graphics::getTexture(registry, normalMap);
	instance->setParameter("normalMap", normalMapTexture.value_or(*dummyWhite), gDefaultSampler);
	...
	instance->setParameter("heightMap", heightMapTexture.value_or(*dummyBlack), gDefaultSampler);
}
...
void SkyBoxMaterial::apply(const graphics::SharedMaterialInstance& instance, const ecs::Registry& registry) const
{
	...
	instance->setParameter("skybox", texture.value_or(std::move(*dummy)), gDefaultSampler);
}
